...
const MyTask = () => {
  ...
  return (
    <div className="lg:m-20 sm:m-0 ">
      <h1 className="text-3xl font-bold mb-4 text-blue-800 font-serif sm:pl-10">
        My Task
      </h1>
      <ul role="list" className="divide-y divide-gray-400  ">
        {state?.reverse().map((person) => (
          <li key={person.title} className="flex justify-between gap-x-6 py-5 rounded-lg shadow-md mt-5">
            <div className="flex min-w-0 gap-x-4 ">
              <img
                className="h-12 w-12 flex-none rounded-full bg-gray-50 sm:mr-10"
                src={
                  person.profileImg
                    ? person.profileImg
